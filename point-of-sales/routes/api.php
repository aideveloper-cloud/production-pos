...
use App\Http\Controllers\Api\Integrations\SpoolmanSyncController;
use App\Http\Controllers\Api\Integrations\SpoolmanWebhookController;
use App\Http\Controllers\Api\PaymentWebhookController;
use App\Http\Controllers\Api\PosApiController;
use App\Http\Controllers\Api\ProductController;
use App\Http\Controllers\Api\SupplierController;
use App\Http\Controllers\Api\WarehouseController;
use App\Http\Controllers\Api\WarehouseSpoolController;
use App\Http\Middleware\VerifySpoolmanIntegrationToken;
...
// Spoolman → WMS integration (service token; Spoolman has no built-in auth)
Route::prefix('integrations/spoolman')
    ->middleware([VerifySpoolmanIntegrationToken::class, 'throttle:60,1'])
    ->group(function () {
        Route::get('/health', [SpoolmanSyncController::class, 'health'])
            ->name('integrations.spoolman.health');
        Route::post('/events', [SpoolmanWebhookController::class, 'store'])
            ->name('integrations.spoolman.events');
        Route::post('/sync', [SpoolmanSyncController::class, 'sync'])
            ->name('integrations.spoolman.sync');
    });
...
Route::prefix('v1')->group(function () {
    ...
    Route::middleware(['auth:sanctum', 'throttle:api'])->group(function () {
        ...
        Route::get('warehouses/{warehouse}/spools', [WarehouseSpoolController::class, 'index'])
            ->middleware('abilities:warehouses-access')
            ->name('api.warehouses.spools.index');
        Route::get('warehouses/{warehouse}/spools/{unitCode}', [WarehouseSpoolController::class, 'show'])
            ->middleware('abilities:warehouses-access')
            ->name('api.warehouses.spools.show');
