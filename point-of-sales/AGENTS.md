...
**No default users.** Admin account, store profile, business type, categories, and main warehouse are created via the first-install setup wizard at `/setup`. Open the root URL after migration; it automatically redirects to `/setup` while `Setting::app_setup_completed` is false. The `setup.notinstalled` middleware returns 404 for `/setup` once setup is done.
...
**Warehouse mode (`APP_MODE=warehouse`, production):** `stock_ledgers` is the source of truth. `product_warehouse.stock` and `products.stock` are caches recomputed from it, and for tracked products (`tracks_physical_units` / `spoolman_enabled`) the ACTIVE `physical_units` of a product in a warehouse add up to its ledger balance.

- **Never write stock directly** (`increment('stock')`, `updateOrCreate(['stock' => …])`). Document flows go through `App\Services\Inventory\InventoryPostingService` (`receive`, `issue`, `adjustTo`, `dispatch` / `arrive` / `returnFromTransit` for transfers) with a `PostingContext` naming the source document. The stock floor uses `WarehouseStockMovementService`, which shares the posting service's cache refresh.
- Outbound stock of tracked products is taken FIFO (oldest unit first, optional preferred lot). Transfers move whole units as they are (label/spool stay valid) and split partial takes into a new `IN_TRANSIT` unit, recorded in `stock_transfer_units`.
- Spoolman is only told after the transaction commits; the ledger wins if the two disagree.
- `php artisan inventory:ledger-check` verifies the four invariants (`--fix-cache` rewrites the caches from the ledger, never the ledger itself). History UI: `stock-ledgers.index`.

**Retail mode:** checkout / dine-in / sales returns still write `product_warehouse.stock` directly, so there the posting service moves the cache by the same delta instead of recomputing it. Use `inventory:reconcile --fix` to align `products.stock` with the per-warehouse totals after data repairs.
