        ...
        return retry(3, function () use ($order, $items, $notes, $userId) {
            return DB::transaction(function () use ($order, $items, $notes, $userId) {
                ...
                foreach ($items as $item) {
                    ...
                    if ($order->warehouse_id && $qtyReceived > 0) {
                        // Ledger + cache; a tracked product gets a new unit lotted by batch or GR number.
                        $this->postingService->receive(
                            ...
                            new PostingContext(
                                ...
                                meta: [
                                    ...
                                    'purchase_order' => $order->document_number,
                                ],
                            ),
                            null,
                            $item['batch_number'] ?? $receiving->document_number,
                        );
                    } elseif (! $order->warehouse_id) {
                        // Legacy single-warehouse installs keep only the global counter.
