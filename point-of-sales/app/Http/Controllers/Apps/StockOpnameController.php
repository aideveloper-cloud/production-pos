        ...
        DB::transaction(function () use ($request, $stockOpname) {
            foreach ($stockOpname->items as $item) {
                ...
                if ($stockOpname->warehouse_id) {
                    ...
                    // Posts only the difference against the balance *now* (stock may have moved since
                    // the count started); tracked products lose FIFO / gain a new unit lotted by the count code.
                    $this->postingService->adjustTo(
                        $product,
                        ...
                        new PostingContext(
                            ...
                            notes: $item->adjustment_reason ?: 'Stock opname '.$stockOpname->code,
                            userId: $request->user()?->id,
                            meta: ['document_number' => $stockOpname->code],
                        ),
                        $stockOpname->code,
                    );
